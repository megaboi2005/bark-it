from flask import render_template, send_file, send_from_directory, Flask, redirect, url_for, abort, request
import json
...
def genposts(page):
	...
	for a in range(3):
		...
		pcount = a+int(page)
		try:
			output += post.replace('^user^',str(json.loads(loadjson())[f"{pcount}"]["author"])).replace('^title^',str(json.loads(loadjson())[f"{pcount}"]["title"])).replace('^content^',str(json.loads(loadjson())[f"{pcount}"]["content"])).replace('{PostID}',str(pcount))
		except KeyError:
			pass
...
def genpost(id):
	...
	return post.replace('^user^',postload["author"]).replace('^title^',postload["title"]).replace('^content^',postload["content"]).replace('{PostID}',id)
def getData(file):
...
def main(name=None):
	...
	return resp.replace('^render^',str(processing_time)).replace('^right^','/posts/1').replace('^left^','/posts/1')
...
def user():
	if request.method == 'POST':
		...
		posts.update({newPostId:{"title":title, "author":"TESTIFICATE", "content":content, "likes":1, "locked":False, "comments":{}}})
